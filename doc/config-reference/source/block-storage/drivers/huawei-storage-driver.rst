...
.. list-table:: **Version mappings among the Block Storage driver and Huawei
   ...
   * - Create, delete, expand, attach, detach, manage and unmanage volumes
       ...
       Create, delete, manage, unmanage, and back up snapshots
...
#. Configure the ``cinder.conf`` file.
   ...
   * ``volume_backend_name`` indicates the name of the backend.
   ...
   in target back-end block for ``Hypermetro``.
...
#. Create a port group on the storage device using the ``DeviceManager``
   and add service links that require multipathing into the port group.
