...
.. list-table:: Description of RabbitMQ configuration options
   ...
   * - ``ca_certs`` =
     - (String) SSL cert file (valid only if SSL enabled).
   * - ``host`` = ``localhost``
     - (String) The RabbitMQ broker address which used for communication with Murano guest agents.
   * - ``insecure`` = ``False``
     - (Boolean) This option explicitly allows Murano to perform "insecure" SSL connections to RabbitMQ
   * - ``login`` = ``guest``
     - (String) The RabbitMQ login.
   * - ``password`` = ``guest``
